...
* $d(x) > 0$ ⇒ 正確累積器變快（$z_1$ 變小）、錯誤累積器變慢 ⇒ 正確率上升。
  「變快 / 變慢」在這裡是**整條分佈**的意義，不只是平均值：因為
  $T = \psi + \exp(z)\cdot W$（$W \sim \text{LogNormal}(0,\sigma^2)$ 與 $z$ 無關），
  $z$ 只是一個乘法縮放因子。
* $d(x) = 0$ ⇒ 兩個累積器變成 iid ⇒ 正確率恰為 50%（純猜測）。
  這條只靠**對稱性**，不需要對數常態這個假設。
* 刺激強度的效果不必是線性的：`alpha2` 讓它可以是一條**拋物線**，
  能捕捉常見的「一開始效果很大、後來飽和」的心理物理曲線形狀。

**RT 的方向要小心。** 觀察到的 RT 是 $\min(T_1, T_2)$，而 $d$ 上升時
$T_1$ 變快、$T_2$ 變慢——`min` 對兩個引數都單調遞增，所以一邊往下推、一邊往上推，
**方向不是自動成立的，必須實際計算**。算出來的結果是：

| $d$ | 正確率 | 全部 RT 中位數 | 正確反應 RT | 錯誤反應 RT |
|---|---|---|---|---|
| 0.0 | 0.501 | 3.353 | 3.350 | 3.357 |
| 0.3 | 0.761 | 3.111 | 2.946 | 3.639 |
| 1.0 | 0.991 | 1.770 | 1.760 | 4.076 |
| 1.5 | 1.000 | 1.119 | 1.119 | 4.144 |

**整體 RT 與正確反應 RT 隨 $d$ 單調下降，但錯誤反應 RT 是反向上升的。**
所以「刺激越明顯反應越快」這句話只對整體與正確反應成立，對錯誤反應不成立。
（這個「錯誤比正確慢、且差距隨 $d$ 擴大」的預測，正是 §10.3(b) 提到、
只擬合正確率就會整個丟掉的那個可檢驗特徵。）
